/**
 * Welcome to discreture. This is a short tutorial on some of the functions provided by discreture. 
 * Remember that discreture is a header-only library, which means there is no need to install anything: 
 * You can simply copy the folder "include" to your project and start using it right away.
 * 
 * It's assumed you already know C++. Compile this file with "g++ -std=c++14 tutorial.cpp -o tutorial"
 * 
 * If you get an error "fatal error: discreture.hpp: No such file or directory" it means your compiler
 * doesn't know where to find discreture. Fix it by compiling with "g++ -std=c++14 -I/path/to/discreture/include tutorial.cpp -o tutorial"
 * 
...
int main()
{
	 ...
	 *   2. It does not check for repeats! So if you do compound_combinations on "aaaaa", 
	 *      it will just produce many sets with "aaa".
	 *   3. The std::string is necessary, because "abcde" is not a proper container. It's an C-style char array.
	 *   4. Note that auto& combination does NOT compile. 
	 * 	    This is because "combination" is of type dscr::arrangement, which vanishes.
	 */
	...
	cout << "\nNow, we'll do it with direct access. Note this is much slower than standard iteration." << endl;

	for (int i : indices(X)) 
		cout << X[i] << endl;
	// indices just gives the container with elements {0,1,2,...,Combinations.size()-1}. 
	// It works with any container that provides the "size()" method.
	
	cout << "\nAll of discreture's containers work the same way.\n\nHere we iterate over partitions, which are all the ways of adding up to a number." << endl;
	...
	cout << "\nWe now do something more complicated: Let's find all combinations for which each element divides the next." << endl;
	...
	auto divides = [](const auto& partialcomb)
	{
		...
		if (k < 2)
			return true;
		return partialcomb[k-2] != 0 && partialcomb[k-1]%partialcomb[k-2] == 0;
