...
public:
    ...
    AggregationViewByRef(const RAContainer& objects, const RAIndexContainer& indices)
...
private:
    const RAContainer& objects_;
    const RAIndexContainer& indices_;
};

// Utility function for C++14 and below, like make_shared. Deprecated in C++17.
template <class RAContainer, class RAIndexContainer>
auto aggregation_view_by_ref(const RAContainer& objects,
                      const RAIndexContainer& indices)
{
    return AggregationViewByRef<RAContainer, RAIndexContainer>(objects, indices);
}

template <class RAContainer, class RAIndexContainer>
std::ostream&
operator<<(std::ostream& os,
           const AggregationViewByRef<RAContainer, RAIndexContainer>& A)
{
    for (auto& a : A)
        os << a << ' ';
    return os;
