...
namespace detail
{
	...
	void combination_tree_helper10(Func f, IntType n)
	{
		...
		for (x[8] = x[7]+1; x[8]+1 < n; ++x[8])
		for (x[9] = x[8]+1; x[9]+0 < n; ++x[9])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper11(Func f, IntType n)
	{
		...
		for (x[8] = x[7]+1; x[8]+2 < n; ++x[8])
		for (x[9] = x[8]+1; x[9]+1 < n; ++x[9])
		for (x[10] = x[9]+1; x[10]+0 < n; ++x[10])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper12(Func f, IntType n)
	{
		...
		for (x[6] = x[6]+1; x[6]+5 < n; ++x[6])
		for (x[7] = x[7]+1; x[7]+4 < n; ++x[7])
		for (x[8] = x[8]+1; x[8]+3 < n; ++x[8])
		for (x[9] = x[9]+1; x[9]+2 < n; ++x[9])
		for (x[10] = x[10]+1; x[10]+1 < n; ++x[10])
		for (x[11] = x[11]+1; x[11]+0 < n; ++x[11])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper13(Func f, IntType n)
	{
		...
		for (x[6] = x[6]+1; x[6]+6 < n; ++x[6])
		for (x[7] = x[7]+1; x[7]+5 < n; ++x[7])
		for (x[8] = x[8]+1; x[8]+4 < n; ++x[8])
		for (x[9] = x[9]+1; x[9]+3 < n; ++x[9])
		for (x[10] = x[10]+1; x[10]+2 < n; ++x[10])
		for (x[11] = x[11]+1; x[11]+1 < n; ++x[11])
		for (x[12] = x[12]+1; x[12]+0 < n; ++x[12])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper14(Func f, IntType n)
	{
		...
		for (x[1] = x[0]+1+1; x[1]+12 < n; ++x[1])
		for (x[2] = x[1]+1+1; x[2]+11 < n; ++x[2])
		for (x[3] = x[2]+1+1; x[3]+10 < n; ++x[3])
		for (x[4] = x[3]+1+1; x[4]+9 < n; ++x[4])
		for (x[5] = x[4]+1; x[5]+8 < n; ++x[5])
		for (x[6] = x[6]+1; x[6]+7 < n; ++x[6])
		for (x[7] = x[7]+1; x[7]+6 < n; ++x[7])
		for (x[8] = x[8]+1; x[8]+5 < n; ++x[8])
		for (x[9] = x[9]+1; x[9]+4 < n; ++x[9])
		for (x[10] = x[10]+1; x[10]+3 < n; ++x[10])
		for (x[11] = x[11]+1; x[11]+2 < n; ++x[11])
		for (x[12] = x[12]+1; x[12]+1 < n; ++x[12])
		for (x[13] = x[13]+1; x[13]+0 < n; ++x[13])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper15(Func f, IntType n)
	{
		...
		for (x[6] = x[6]+1; x[6]+8 < n; ++x[6])
		for (x[7] = x[7]+1; x[7]+7 < n; ++x[7])
		for (x[8] = x[8]+1; x[8]+6 < n; ++x[8])
		for (x[9] = x[9]+1; x[9]+5 < n; ++x[9])
		for (x[10] = x[10]+1; x[10]+4 < n; ++x[10])
		for (x[11] = x[11]+1; x[11]+3 < n; ++x[11])
		for (x[12] = x[12]+1; x[12]+2 < n; ++x[12])
		for (x[13] = x[13]+1; x[13]+1 < n; ++x[13])
		for (x[14] = x[14]+1; x[14]+0 < n; ++x[14])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper16(Func f, IntType n)
	{
		...
		for (x[6] = x[6]+1; x[6]+9 < n; ++x[6])
		for (x[7] = x[7]+1; x[7]+8 < n; ++x[7])
		for (x[8] = x[8]+1; x[8]+7 < n; ++x[8])
		for (x[9] = x[9]+1; x[9]+6 < n; ++x[9])
		for (x[10] = x[10]+1; x[10]+5 < n; ++x[10])
		for (x[11] = x[11]+1; x[11]+4 < n; ++x[11])
		for (x[12] = x[12]+1; x[12]+3 < n; ++x[12])
		for (x[13] = x[13]+1; x[13]+2 < n; ++x[13])
		for (x[14] = x[14]+1; x[14]+1 < n; ++x[14])
		for (x[15] = x[15]+1; x[15]+0 < n; ++x[15])
		{
			f(x);
		}
	}
	...
	void combination_tree_helper17(Func f, IntType n)
	{
		...
		for (x[8] = x[7]+1; x[8]+8 < n; ++x[8])
		for (x[9] = x[8]+1; x[9]+7 < n; ++x[9])
		for (x[10] = x[9]+1; x[10]+6 < n; ++x[10])
		for (x[11] = x[10]+1; x[11]+5 < n; ++x[11])
		for (x[12] = x[11]+1; x[12]+4 < n; ++x[12])
		for (x[13] = x[12]+1; x[13]+3 < n; ++x[13])
		for (x[14] = x[13]+1; x[14]+2 < n; ++x[14])
		for (x[15] = x[14]+1; x[15]+1 < n; ++x[15])
		for (x[16] = x[15]+1; x[16] < n; ++x[16])
		{
			f(x);
		}
	}
